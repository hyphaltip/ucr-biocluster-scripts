...
#    $bdir =~ s/\/$//;
    ...
    print $jobfh "if [ ! -f $base.intervals ]; then\n  java $javamem -jar $gatk_jarfile -T RealignerTargetCreator -R $reference_genome -o $base.intervals -I $fname; fi\n";

    if( $force ) {
	unlink("$bdir/$base.realign.bam");
    }    
    print $jobfh "if [ ! -f $base.realign.bam ]; then\n  java $javamem -jar $gatk_jarfile -T IndelRealigner -R $reference_genome -targetIntervals $base.intervals -I $fname -o $base.realign.bam; fi\n";
    
    print $jobfh "if [ ! -f $base.snps.raw.vcf ]; then\n  java $javamem -jar $gatk_jarfile -T UnifiedGenotyper -R $reference_genome -I $base.realign.bam -o $base.snps.raw.vcf; fi\n";
}
